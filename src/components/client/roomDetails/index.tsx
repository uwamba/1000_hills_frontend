...
import BookingForm from '../bookingForm'; // adjust this path as needed

interface User { id: number; name: string; }
interface Hotel { id: number; name: string; }
interface Photo { id: number; path: string; }
    ...
    <div className="p-6 bg-gray-50 min-h-screen max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
        ...
        <div>
          <div className="w-full h-[400px] mb-4 rounded-xl overflow-hidden shadow-lg border border-gray-300">
            <img
              src={photos.length > 0 ? `${imageBaseUrl}/${photos[selectedImageIndex].path}` : '/placeholder.jpg'}
              alt="Selected"
        ...
        <div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-800 mb-6">
            ...
            <p><strong>AC:</strong> {room.has_ac ? 'Yes' : 'No'}</p>
            <p>
          ...
          {/* BookingForm now receives both ID and price */}
          <BookingForm propertyId={room.id.toString()} price={room.price} object_type='room'/>
        </div>
      </div>
      ...
      <div className="mt-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {similarRooms.length ? (
            similarRooms.map((sr) => (
              <div key={sr.id} className="bg-white rounded shadow-lg p-4">
                <img
                  src={`${imageBaseUrl}/${sr.photos?.[0]?.path || '/placeholder.jpg'}`}
                  alt={sr.name}
                  className="w-full h-48 object-cover rounded"
                />
                <h3 className="text-lg font-semibold text-indigo-700 mt-4">{sr.name}</h3>
                <p className="text-gray-700">Price: {sr.price} {sr.currency}</p>
              </div>
            ))
          ) : (
